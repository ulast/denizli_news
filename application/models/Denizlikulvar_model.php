...
/**
 * Proje : mld
 * Tarih : 25.09.2014 - 10:11
 * Mail  : ulas@mail.com
 * Web   : http://www.red.net.tr
 */

class Denizlikulvar_model extends MY_Model
{
    public function listele($gun,$ay,$yil,$saat)
    {
        $url    =   "http://www.denizlikulvargazetesi.net/feed";
        ...
        if($xml)
        {
            ...
            foreach($items as $item)
            {
                ...
                if($int_tarih > $bugun)
                {
                    $html = str_get_html($item->children("content",true));
                    ...
                    $liste[$i]['source']        =   4;
                    $liste[$i]['image']         =   $html->find('img', 0)->attr['src'];
            ...
            if(isset($liste))
            {
                ...
                $return .= "</table>";
                $return .=  "<input type='hidden' name='source' value='4'>";
            }
            else
            {
